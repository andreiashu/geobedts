...
export async function loadMaxMindCities(path) {
    const cities = [];
    const dedupeIdx = new Map();
    const locationDedupeIdx = new Set();
    if (!existsSync(path))
        return cities;
    return new Promise((resolve, reject) => {
        ...
        rl.on('close', () => {
            for (const fields of dedupeIdx.values()) {
                ...
                if (locationDedupeIdx.has(dedupeKey))
                    continue;
...
export function loadGeonamesCountryInfo(path) {
    const countries = [];
    ...
    for (const line of lines) {
        if (!line || line[0] === '#')
            continue;
        const fields = line.split('\t');
        if (fields.length !== 19 || fields[0] === '' || fields[0] === '0')
            continue;
        countries.push({
            iso: fields[0],
            iso3: fields[1],
